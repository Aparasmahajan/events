...
- **Phase 2 — Templates (static, exceptional):** registry + Royal wedding fully, plus 1–2 distinct templates; message/override + theming on dummy data; hit the §8 bar.
  - **Template catalog (42, as of 2026-07-05):** all live under `components/templates/<id>/index.tsx`, registered in `components/templates/TemplateRouter.tsx`, discoverable via `components/templates/metadata.ts`. Each has a demo bundle in `lib/dummyData.ts` keyed by `DEMO-<TEMPLATE>` and previewed by `scripts/generate-template-previews.mjs`.
    - Wedding: **royal** (big-fat-Indian), **minimal** (whitespace/editorial), **aurora** (cinematic-celestial), **obsidian** (black-tie), **celestia** (ethereal), **pastel** (soft-romance), **empyrean** (divine-marble), **prism** (crystal-iridescent), **moonlit** (fantasy kingdom, moon + lanterns), **skytemple** (floating cloud temples), **oceanpalace** (underwater palace, pearl bubbles), **symphony** (wedding as musical score).
    - Engagement: **promise** (split-names converge on scroll), **infinity** (two rings converge into ∞), **lovestars** (memories as constellation + proposal galaxy-burst), **garden** (botanical blooms + butterflies), **horizon** (sunset→twilight scroll-morphing sky).
    - Anniversary: **chapters** (parchment journal), **library** (infinite library, book-spine gallery, letter-butterflies), **timemachine** (clock-dial hero, polaroid years — also milestone birthdays).
    - Birthday & party: **vibrant** (kids/pink-confetti), **orbit** (cosmic-kids), **arcade** (80s synthwave), **toybox** (toy city, train + present-box gallery), **carnival** (neon fair, ferris wheel + fireworks), **dreamfactory** (imagination factory, conveyor + gears), **modern** (dark bold), **after** (Berlin nightlife), **metropolis** (Tokyo-cyberpunk skyline), **infinityclub** (endless megaclub, tunnel + LED walls).
    - Corporate, launch, awards, networking: **pinnacle** (executive summit), **nexus** (cyberpunk launch), **neural** (AI-summit / research), **quantum** (smart-city conference, skyline of tracks), **unveil** (dramatic reveal keynote), **genesis** (particles-assemble-the-product launch), **luminary** (celestial gala), **odeon** (Oscars/red-carpet awards), **immortals** (monumental hall, gold pillars + reflective floor), **converge** (warm networking), **constella** (constellation networking), **ecosystem** (living-organism network, growing tree).
