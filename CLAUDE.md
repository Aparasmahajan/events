...
- **Phase 2 — Templates (static, exceptional):** registry + Royal wedding fully, plus 1–2 distinct templates; message/override + theming on dummy data; hit the §8 bar.
  - **Template catalog (80, as of 2026-07-07):** all live under `components/templates/<id>/index.tsx`, registered in `components/templates/TemplateRouter.tsx`, discoverable via `components/templates/metadata.ts`. Each has a demo bundle in `lib/dummyData.ts` keyed by `DEMO-<TEMPLATE>` and previewed by `scripts/generate-template-previews.mjs`.
    - Wedding (20): **royal** (big-fat-Indian), **minimal** (whitespace/editorial), **aurora** (cinematic-celestial), **obsidian** (black-tie), **celestia** (ethereal), **pastel** (soft-romance), **empyrean** (divine-marble), **prism** (crystal-iridescent), **moonlit** (fantasy kingdom, moon + lanterns), **skytemple** (floating cloud temples), **oceanpalace** (underwater palace, pearl bubbles), **symphony** (wedding as musical score), **skyrealm** (floating kingdoms + marble bridges + golden birds), **cathedral** (constellation cathedral, stained-glass gallery), **sakura** (cherry-blossom forest, seasons on scroll), **versailles** (French-baroque palace rooms, gilt frames), **fresco** (Renaissance painting, museum plaques + brush-stroke reveals), **mirage** (Arabian desert dunes → moonlit indigo, khatam star), **icepalace** (Nordic aurora + ice-block cards + snowfall), **galaxyopera** (curtain-reveal opera in space, planet chandelier, ACTs I–IV).
    - Engagement (11): **promise** (split-names converge on scroll), **infinity** (two rings converge into ∞), **lovestars** (memories as constellation + proposal galaxy-burst), **garden** (botanical blooms + butterflies), **horizon** (sunset→twilight scroll-morphing sky), **tworivers** (two-river confluence + lotus flowers), **mirrorworlds** (split warm/cool worlds merge into symmetry), **infinitytrain** (Orient-Express memory train), **lanterns** (floating-lantern night lake), **glassrose** (crystal rose blooms on scroll), **secretgalaxy** (nebula + star-dot engagement rings + orbiting asteroids).
    - Anniversary (6): **chapters** (parchment journal), **library** (infinite library, book-spine gallery), **timemachine** (clock-dial hero, polaroid years), **timecapsule** (brass capsule opens, artifacts float up, decade badges), **treeoflife** (central tree grows branches per year), **endlessclock** (multiple bronze clocks + roman-numeral dials, porthole gallery).
    - Birthday (10): **vibrant** (kids/pink-confetti), **orbit** (cosmic-kids), **arcade** (80s synthwave), **toybox** (toy city, train + present-box gallery), **carnival** (neon fair, ferris wheel + fireworks), **dreamfactory** (imagination factory, conveyor + gears), **cartoon** (Pixar-ish clouds + balloon buddies + bouncy underlines), **bricktown** (toy-brick assembly + brick-letter wordmark), **spacemission** (cockpit HUD + orbit planets + rocket-launch CTA), **candyland** (chocolate river + cookie frames + candy-wrapper cards), **robocity** (waving robot + drones + delivery-package cards), **jungle** (temple silhouette + swaying vines + stone tablets), **timemachine** (also birthday milestones), **treasure** (parchment map + coin-flip waypoints + treasure-chest CTA), **themepark** (park skyline + coaster loop + film-strip gallery).
    - Party (7): **modern** (dark bold), **after** (Berlin nightlife), **metropolis** (Tokyo-cyberpunk skyline), **infinityclub** (endless megaclub, tunnel + LED walls), **festival** (Tomorrowland-arch stage + LED flowers + laser tunnels), **neonjungle** (neon-outline leaves + peeking animals + waterfall), **midnighttokyo** (Shibuya rain + neon kanji sign-towers + puddle reflection).
    - Corporate & tech (7): **pinnacle** (executive summit), **nexus** (cyberpunk launch), **neural** (AI-summit / research), **quantum** (smart-city conference, skyline of tracks), **digitalcity** (isometric grid + data-highway bands + address cards), **quantumlab** (molecular hologram + Petri experiments), **missioncontrol** (control-room dashboard + satellite-map venue).
    - Product launch (5): **unveil** (dramatic reveal keynote), **genesis** (particles-assemble-the-product launch), **secretlab** (vault-door unlock + declassified brief + server-module cards), **portal** (portal-ring + chromatic-aberration title + energy tendrils), **evolution** (scroll-driven era gradient + primitive→AI timeline).
    - Awards & networking (11): **luminary** (celestial gala), **odeon** (Oscars/red-carpet awards), **immortals** (monumental hall, gold pillars + reflective floor), **goldenuniverse** (golden planets + crossed spotlights + cosmic-theater CTA), **halloffame** (marble statues + laurel-wreath headings + museum plaques), **converge** (warm networking), **constella** (constellation networking), **ecosystem** (living-organism network, growing tree), **synapse** (deterministic neuron graph + pulse impulses), **futurecity** (growing-skyline SVG + highway arcs + district cards).
  - Signature-motif intent per template is what makes each distinct in the picker grid — keep the motif (lantern-rise for moonlit, prism-beam for prism, particle-assembly for genesis, etc.) when redesigning or regenerating previews.
  - **Landing grid** (`app/(marketing)/_components/LandingTemplates.tsx`) shows a curated first page — 6 on desktop, 2 on mobile — with a "View all templates" toggle. Search or tag filters bypass the cap.
